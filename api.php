...
// Terminated also key off start_date - they have not actually closed, so they
// never have a closure_date (mirrors js/view.js's periodDateOf(), which uses the
...
function getStaffPerformanceLive($month, $year, $quarter, $selectedStatuses, $staff_id, $filterAtemType = 0, $filterOutletId = 0, $filterRoles = null)
{
    ...
    foreach ($listResult['data'] as $item) {
        ...
        // The reward amount is strictly limited to incentivised A/R members on
        // an approved completed-family card (final_incentive_amount > 0, same
        // rule as CalculateBonusEligibility.php) - an issuer, area manager,
        // C/I role, non-incentivised A/R, or an unapproved card all
        // contribute RM0. This does NOT gate the 'complete' COUNT below -
        // that just reflects participation, same as active/extend/failed.
            ...
            switch ($action) {
                ...
                case 'dashboard-stats':
                    ...
                    foreach ($items as $item) {
                        ...
                        // Overdue is a derived state, not a status: an Active or
                        // Extended card whose final_due_date (end_date, or the
                        // extension date once extended) is already in the past.
                        // Closed/Draft/Suspended/Force Terminated cards never count.
                        if ($statusVal === 'Active' || $statusVal === 'Extended') {
                            $finalDue = isset($item['final_due_date']) ? substr((string)$item['final_due_date'], 0, 10) : '';
                            if ($finalDue !== '' && $finalDue < $todayStr) {
                                $overdueCount++;
                            }
                        }
