...
(function () {
    ...
    function pickCrossScopeStaff(staffId) {
        ...
        if (currentArciScope() === 'outlet') {
            ...
            if (staffType === 'outlet' && $('arci-scope-department')) {
    ...
    function renderStaffList() {
        ...
        if (!deptId) {
            ...
            listDiv.innerHTML = '<div class="text-muted" style="font-size:13px;">Select ' + (isOutletScope ? 'an outlet' : 'a department') + ' to load staff, or type a name to search all staff</div>';
            return;
        }
    ...
    function bind() {
        ...
        if ($('atem-reward-label')) { $('atem-reward-label').addEventListener('change', markChanged); }
